...
class MechanicUserForm2(forms.ModelForm):
    class Meta:
        model=User
        fields=['first_name','last_name','username','password']
        widgets = {
...
class MechanicSalaryForm2(forms.Form):
    salary=forms.IntegerField();
...
class AdminApproveRequestForm(forms.Form):
    mechanic=forms.ModelChoiceField(queryset=models.Mechanic.objects.all(),empty_label="Mechanic Name",to_field_name='id')
    cost=forms.IntegerField()
    stat=(('Pending','Pending'),('Approved','Approved'),('Finished','Finished'))
...
class MechanicUpdateStatusForm(forms.Form):
    stat=(('Approved','Approved'),('Repairing','Repairing'),('Work Done','Work Done'))
    status=forms.ChoiceField( choices=stat)

class MechanicUpdateStatusForm2(forms.Form):
    stat=(('Approved','Approved'),('Repairing','Repairing'),('Work Done','Work Done'))
    status=forms.ChoiceField( choices=stat)
